...
start = {} # dictionary that holds trigram logprobs for valid starts
           # for a sentence
lm = {} # dictionary that holds the rest of the trigram logprobs
...
def generate(numsents, printprob):
  ...
  while numsents > 0:
    ...
    while 1:
      ...
      if not lm.has_key(key):
        print >> sys.stderr, "missing key", key
        break
...
def usage():
  print >> sys.stderr, "usage: python gen-from-lm.py [-p|--printprob] -l|--lmfile file -n|--numsents num"

if __name__ == '__main__':
  try:
    (printprob, lmfile, numsents) = (0, None, 1)
    opts, args = getopt.getopt(sys.argv[1:], "pl:n:", ["printprob", "lmfile=", "numsents="])

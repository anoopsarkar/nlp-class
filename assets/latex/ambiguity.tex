...
\begin{frame}
\frametitle{Inherently Ambiguous Languages}
\begin{itemize}
	\item Consider the following context-free grammar:
		\begin{itemize}
		\item $S \rightarrow S1 \mid S2$
		\item $S1 \rightarrow a X d \mid \epsilon$
		\item $X \rightarrow b X c \mid \epsilon$
		\item $S2 \rightarrow Y Z \mid \epsilon$
		\item $Y \rightarrow a Y b \mid \epsilon$
		\item $Z \rightarrow c Z d \mid \epsilon$
		\end{itemize}
	\item Now parse the input string $abcd$ with this grammar
	\item Notice that we get two parse trees (one with the $S1$ sub-grammar and another with the $S2$ subgrammar).
\end{itemize}

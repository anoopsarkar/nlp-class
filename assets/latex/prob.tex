...
\begin{itemize}[<+->]
\item P(X = \textit{spain}) $= 5.03e-5$
\item P(X = \textit{pain}) $= 4.11e-5$
\item P(X = \textit{choose}) $= 8.78e-5$
\item P(X = \textit{chooses}) $= 3.15e-6$
\end{itemize}
\end{itemize}
\end{frame}
...
\item P(X = \textit{choose}, Y = \textit{spain}) $= 7.71e-9$
\item P(X = \textit{chooses}, Y = \textit{pain}) $= 9.98e-12$
\end{itemize}
\end{itemize}
\pause
...
\item {\color{blue} P(X = \textit{choose}, Y = \textit{spain})} $\Leftrightarrow$ {\color{red} P(Y = \textit{spain}, X = \textit{choose})}
\item But in both cases it is P(X,Y) = P(Y,X) = P(\textit{choose spain})
\end{itemize}
\end{alertblock}
\end{frame}
...
\begin{itemize}[<+->]
\item P(Y $\mid$ X) means probability that Y has a certain value after fixing value of X.
\begin{itemize}[<+->]
\item P(Y= \textit{spain} $\mid$ X = \textit{choose}) $= 8.78e-5$
\item P(Y= \textit{pain} $\mid$ X = \textit{chooses}) $= 3.15e-6$
\end{itemize}
\end{itemize}
\pause
\begin{block}{Conditional probability from Joint probability}
\begin{itemize}
\item P(Y $\mid$ X) = $\frac{\mbox{P(X, Y)}}{\mbox{P(X)}}$
\end{itemize}
\end{block}
\end{frame}

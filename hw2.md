...
* for t = 1, ..., T, for i = 1, ..., n
    * Use the Viterbi algorithm to find the output of the model on the $$i$$-th training sentence
    <p>$$ z_{[1:n]} = \arg\max_{u_{[1:n]} \in {\cal T}^n_i} \sum_s w_s \Phi_s(w_{[1:n_i]}^{(i)}, u_{[1:n_i]}) $$</p>
        * The function `perc_test` in `perc.py` implements the Viterbi algorithm
...
* Use different chunking representations.
* Use a Margin Infused perceptron algorithm.
* Use a Conditional Random Field.
* Use trigram features over output labels instead of bigram features.
...
* You must turn in three things:
  1. Chunking output of the test dataset which is in `chunker/data/input.txt.gz` uploaded to the [leaderboard submission site](http://sfu-nlp-class.appspot.com) according to [the Homework 0 instructions](hw0.html). You can upload new output as often as you like, up until the assignment deadline. The Submit button for showing the test set scores will be unavailable until after the homework deadline and grace days have passed.  The output will be evaluated using a secret metric, but the `score-chunks.py` program will give you a good idea of how well you're doing.
